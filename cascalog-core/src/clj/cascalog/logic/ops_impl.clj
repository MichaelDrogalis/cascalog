...
(defn nil-safe-combiner 
  "Takes a function f of two variables, and returns another function
  of two variables that returns a function that calls f on its
  arguments only if both are non-nil. If one argument is nill, it
  prefers the other argument, and if both are nil it returns nil."
  [f]
  (fn [x y]
    (cond 
     (and (nil? x) (nil? y)) nil
